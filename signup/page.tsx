    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try{
            ...
            if(axios.isAxiosError(err) && err.response){
                ...
                // Handle specific error messages from then backend
                if(status === 400){
                    setError(data.message || 'Invalid email address.');
                }else if(status === 429){
                    setError('An error occurred during sign up. Please try again.');
                }else{
                    setError('An error occurred during up. Please try again.');
                }
            }else {
        ...
        <>
            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
                ...
                <form onSubmit={handleSubmit} className="mt-8" >
                    ...
                    <div>
                        <button type="submit" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-rose-400 hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500 my-8">
                            Sign Up
                        </button>
                    </div>
